...
def uniform_anchors(width, height, M, radius):
    anchors = []
    for i in range(M):
        x, y = uniform_point(width, height)
        sn = Point(x=x, y=y, r=radius, order=i)
        while sn in anchors:
            x, y = uniform_point(width, height)
            sn = Point(x=x, y=y, r=radius, order=i)
        anchors.append(sn)
...
def uniform_non_anchors(width, height, N, M, radius):
    non_anchors = []
    for i in range(N):
        x, y = uniform_point(width, height)
...
if __name__ == '__main__':
    ...
    for name, (fn_s, fn_r) in GENS.items():
        for i in range(args.count):
            ...
            non_anchors_ = fn_r(args.width, args.height, args.non_anchors, args.anchors, args.radius)
